// ✅ RecruitersDataManager.tsx (EXACT OurSchoolsCollegesDataManager UI)
...
const RecruitersDataManager: React.FC = () => {
  ...
  const [title, setTitle] = useState("Our Recruiters");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (dataItem?._id && dataItem.data) {
        setDoc(dataItem as RecruitersDoc);
        setTitle(dataItem.data.title || "Our Recruiters");
      } else {
        const defaultDoc = {
          _id: "default",
          data: { title: "Our Recruiters", logos: [] },
        };
        setDoc(defaultDoc as RecruitersDoc);
  ...
  return (
    <div className="p-6  mx-auto">
      ...
      {/* EDITOR SECTION – EXACTLY MATCHES OurSchoolsCollegesDataManager */}
      <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-10 shadow-sm">
       ...
       <div className="flex justify-between">
        <strong> Recruiters Logos ({doc.data.logos.length})</strong>
         <a className="green-btn " href="#addnew">
          + Add Item
        </a>
       </div>
        ...
        {/* <div className="mb-10 max-w-2xl">
          ...
          <input
            ...
            placeholder="e.g., Our Partner Recruiters"
          />
        </div> */}

        <div className="grid grid-cols-2 md:grid-cols-3 mt-5 py-5 lg:grid-cols-4 xl:grid-cols-4 gap-6">
          ...
          <div
            id="addnew"
            className="border-2 border-dashed border-gray-300 rounded-2xl p-8 bg-gray-50 flex flex-col items-center justify-center hover:border-blue-400 hover:bg-blue-50 transition-all group cursor-pointer h-48 col-span-1"
          >
            ...
            <p className="text-sm text-gray-500 mt-3 text-center font-medium">
              Click to upload new recruiter logo
            </p>
          </div>
        </div>
        ...
        {(!doc?.data.logos || doc.data.logos.length === 0) && (
          <div className="text-center py-16 mt-12 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
            ...
            <p className="text-gray-600 mb-6">
              Add your first recruiter logo above
            </p>
          </div>
        )}
      </div>

      {/* FIXED PREVIEW SECTION – SAME WRAPPING, different public component */}
      ...
      {/* POPUPS – texts adjusted for Recruiters */}
      <ConfirmationPopup
        isOpen={showAddPopup}
        title="Add New Logo?"
        message="This logo will be saved immediately to your Recruiters section."
