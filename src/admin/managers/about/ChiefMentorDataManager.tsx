...
const ChiefMentorDataManager: React.FC = () => {
  ...
  const [originalItem, setOriginalItem] = useState<ChiefMentorData | null>(
    null
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const fileName of tempFiles) {
        ...
        if (editItem.data.content.cheifmentordeskimage === fileName) {
          editItem.data.content.cheifmentordeskimage = finalUrl;
        }
        if (editItem.data.content.ourleadsimage === fileName) {
          editItem.data.content.ourleadsimage = finalUrl;
        }
    ...
    return (
      <div style={{ gap: "2rem", position: "relative" }}>
        ...
        <div className="form-container border border-gray-300 rounded-lg p-6 bg-white">
          <Heading
            title={isNew ? "Create New" : "Edit Chief Mentor Data"}
            size="lg"
            align="left"
          />

          <div className="mb-8 p-4 bg-gray-50 rounded-lg mt-6">
            ...
            <ImageUploadManager
              label="Chief Mentor Desk Image"
              value={editItem.data.content.cheifmentordeskimage}
              onChange={(v) =>
                updateField("data.content.cheifmentordeskimage", v)
              }
              addTemp={addTempFile}
            />
            ...
            <div className="mt-6">
              <div className="flex justify-between items-center mb-3">
                <strong className="text-sm">
                  Paragraphs ({editItem.data.content.paragraphs.length})
                </strong>
  ...
  return (
    <div className="collection-manager p-6">
      ...
      <ConfirmationPopup
        ...
        title="Save Changes?"
        message="Are you sure you want to save these changes? This will update the Chief Mentor data permanently."
